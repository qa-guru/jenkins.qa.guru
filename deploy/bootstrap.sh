...
install -d -o "${DEPLOY_USER}" -g docker -m 775 "${CONFIG_DIR}/java-jdk21-agent" "${CONFIG_DIR}/java-jdk17-agent" "${CONFIG_DIR}/python-python314-agent" "${CONFIG_DIR}/js-node24-agent"

echo "=== ownership for deploy user ${DEPLOY_USER} ==="
chown -R "${DEPLOY_USER}:docker" "${CONFIG_DIR}"
chmod 775 "${CONFIG_DIR}" "${CONFIG_DIR}/bin" "${CONFIG_DIR}/java-jdk21-agent" "${CONFIG_DIR}/java-jdk17-agent" "${CONFIG_DIR}/python-python314-agent" "${CONFIG_DIR}/js-node24-agent" 2>/dev/null || true
